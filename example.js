...
const StatTrak = require('./src/stattrak');
...
const testBlocks = [
    ...
    // Web Explorer: https://blockchair.com/bitcoin/block/50001
    {
        ...
        initialNonce: 52017043 // Correct nonce will be: 56717043
    },
    ...
    // Raw JSON View: https://learnmeabitcoin.com/explorer/block/json.php?hash=000000000000000000072f7aa71b04436897ee8e68e01bc94874fc111ee616fc
    {
        ...
        initialNonce: 0//3396504747 // Correct nonce will be: 3398594747
    },
    // First testnet block!
    {
        block: {
        ...
        initialNonce: 0 // Correct nonce will be: not a clue!
    }
];
...
function createThread(nonce = new Number(), jump = new Number(), id) {
    return new worker_thread.Worker("./mining-thread.js", {
        workerData: { block, nonce, jump, id },
    });
}

// Entry point for worker data reports
let fDoneFirstBatch = false;
function ProcessThreadData(data) {
    // 1: Hashrate Reporting
    if (data.type === 1) {
        Stats.addHashes(data.hashes);
        // Start tracking performance metrics after the first report
        if (!fDoneFirstBatch) {
            fDoneFirstBatch = true;
            Stats.startTime = Date.now();
        }
    } else
    // 2: Block Finalization
    if (data.type === 2) {
        // Verify the nonce wins the block
        if (miner.verifyNonce(block, data.nonce)) {
            console.log('Complete in ' + chalk.green(((Date.now() - Stats.startTime) / 1000) + 's') + ' by ' + chalk.magenta('core ' + data.worker + '!'));
            clearInterval(Tracker);
            workers.map(a => a.terminate());
        }
    } else
    // 3: Data Log
    if (data.type === 3) console.log(data);
}

// Setup Global StatTrak
const Stats = new StatTrak();
...
while (workers.length < Threads) {
    ...
    //                        Start,       Jump,    ID
    workers.push(createThread(nonce + len, Threads, len + 1));
}
workers.map(a => a.on('message', ProcessThreadData));

// Track threads and aggregate stats
const Tracker = setInterval(() => {
    if (fDoneFirstBatch)
        miner.progressReport(Stats.getHashes(), Stats.getElapsedTime());
}, 2500);